...
int Aij(int A[], int i, int j, int k, int l, int arSize[], int &nValue)
{	// 적절한 색인이면 nValue에 값을 저장하고 T/F를 반환한다.v
	if (i < 0 || i >= arSize[0] || j < 0 || j >= arSize[1] || 
		k < 0 || k >= arSize[2] || l < 0 || l >= arSize[3])
		return false;
	int nNdx = i * arSize[1] * arSize[2] * arSize[3] +
		   j * arSize[2] * arSize[3] +
		   k * arSize[3] +
		   l * 1;			// [i][j][k][l] arSize[0], arSize[1], arSize[2], arSize[3]
	nValue = A[nNdx];
	return true;
}
